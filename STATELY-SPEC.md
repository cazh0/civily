# Stately — Technical Specification
...
| Tree | What | Status |
|---|---|---|
| `Stately/` | Java + Views app, 31.8k LOC. Shipping. | Frozen reference. Bug fixes only. |
| `stately-app/` | Kotlin + Compose rebuild from zero. | Active. Own `README.md` documents its structure. |
...
- New dependency — except to replace a dead one. New module. Further architecture migration beyond the instructed `stately-app/` rewrite.
- Any endpoint other than nationstates.net. Analytics, crash SDKs, ads.
- Undocumented NS shards.
- Bypassing the rate limiter or the credential store — `DashHelper.addRequest` / `PinkaHelper` in `Stately/`, `RateLimiter` / `SessionStore` in `stately-app/`. In `stately-app/` both are reachable only through `NsClient`; opening a connection anywhere else is the violation.
...
- Colour / radius / size literals outside `res/values` (`Stately/`) or `ui/theme` (`stately-app/`).
...
- Growing `SparkleHelper` — already a 1664-line god class. No equivalent may appear in `stately-app/`.
...
- `stately-app/`: request results land in a ViewModel-owned `StateFlow`, collected with `collectAsStateWithLifecycle`. No callback may hold a view. This is what closes the crash class above; a screen that reintroduces a view-holding callback is the defect.
- Catching is allowed only at parse/IO boundaries, and must both log **and** surface a visible failure state. Silent failure = defect. In `stately-app/` those boundaries are `NsClient.execute` and `decodeNsXml` — add no third.
...
- Unit tests mandatory on parsing, rate limiting, and BBCode/HTML.
  ...
  - `stately-app/` — `RateLimiterTest`, `NationDtoTest`, `RegionDtoTest`, `AutologinTokenTest`, `PercentCodecTest`, `NsIdTest`, `PopulationTest`, `AssemblyDtoTest`, `BbParserTest`, `BbColorTest`, `HtmlEntitiesTest`, `RelativeTimeTest`, `RmbDtoTest`, `IssueDtoTest`, `NsTextTest`, `InitialsTest`, `NewspaperTest`, `IssueResultDtoTest`, `PercentTest` exist (142 tests). Every new DTO ships a parser test including a missing-element case.
- One PR, one concern.
...
`stately-app/` wires its object graph by hand in `AppGraph`. Reach for a DI framework only when that file stops fitting on one screen.

**Absence is decision, not oversight (R1).** Gaps in the rewrite are listed in `stately-app/README.md`; nothing there is stubbed to look like it works.

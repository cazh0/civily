...
class NsClient(
    ...
    suspend fun signIn(url: HttpUrl, password: String): Outcome<SignIn> {
        ...
        return when {
            outcome is Outcome.Failure && outcome.error == StatelyError.Unauthorized ->
                Outcome.Failure(StatelyError.InvalidLogin)
    ...
    private suspend fun execute(
        ...
        try {
            http.newCall(request).execute().use { response ->
                ...
                when {
                    ...
                    response.code == HTTP_UNAUTHORIZED || response.code == HTTP_FORBIDDEN -> {
                        // Why the session is dropped rather than merely reported: the API
                        ...
                        if (adoptCredentials && session.current != null) session.signOut()
                        Outcome.Failure(StatelyError.Unauthorized)
                    }

                    response.code == HTTP_NOT_FOUND -> Outcome.Failure(StatelyError.NotFound)
                    ...
                    response.code == HTTP_TOO_MANY_REQUESTS -> {
                        ...
                        Outcome.Failure(StatelyError.RateLimited)
                    }

                    response.code == HTTP_CONFLICT -> Outcome.Failure(StatelyError.LoginConflict)

                    else -> Outcome.Failure(StatelyError.Server(response.code))
                }
            }
        } catch (e: IOException) {
            ...
            Outcome.Failure(StatelyError.NoConnection)
        }
    }

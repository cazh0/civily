 ...
 */
sealed class StatelyError(@StringRes val messageRes: Int) {
    ...
    data object NoConnection : StatelyError(R.string.error_no_connection)

    /** Autologin or PIN rejected. The session needs re-establishing. */
    data object Unauthorized : StatelyError(R.string.error_unauthorized)
    ...
    data object InvalidLogin : StatelyError(R.string.error_invalid_login)

    /** The nation, region or resolution does not exist. */
    data object NotFound : StatelyError(R.string.error_not_found)
     ...
     */
    data object RateLimited : StatelyError(R.string.error_rate_limited)
     ...
     */
    data object LoginConflict : StatelyError(R.string.error_login_conflict)

    /** NationStates answered, but with an error status. */
    data class Server(val code: Int) : StatelyError(R.string.error_server)

    /** The response arrived but did not parse. [detail] is for the log, never for the user. */
    data class Malformed(val detail: String) : StatelyError(R.string.error_malformed)
}

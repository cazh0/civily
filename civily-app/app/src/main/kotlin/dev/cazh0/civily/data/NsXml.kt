...
/** [detail] reaches the log only; the user sees the generic message on [CivilyError.Malformed]. */
fun malformed(tag: String, cause: Throwable): CivilyError.Malformed {
    Log.w("NsXml", "$tag response did not parse", cause)
    return CivilyError.Malformed(cause.message.orEmpty())
}
